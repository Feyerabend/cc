...
static void draw_game_to_buffer(void) {
    ...
    // Draw messages
    if (game_over) {
        buffer_draw_string(90, 60, "GAME OVER", COLOR_RED, COLOR_BLACK);
        buffer_draw_string(65, 75, "PRESS X TO RESTART", COLOR_WHITE, COLOR_BLACK);
    } else if (!ball_launched) {
        buffer_draw_string(70, 60, "PRESS X TO LAUNCH", COLOR_WHITE, COLOR_BLACK);
    }
}
