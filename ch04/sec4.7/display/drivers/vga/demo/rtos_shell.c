...
/*
 * fault_test - deliberately corrupt a task's stack guard word to
 * demonstrate overflow detection.  Usage: fault_test [task_index]
 *
 * On the next context switch AWAY from that task, pendsv_switch()
 * will find the corrupted guard, suspend the task, and record the
 * fault.  The display overlay will appear within ~50 ms.
 *
 * This is an educational demo; on a hardened system this command
 * would be gated behind #ifdef SHELL_UNSAFE_CMDS.
 */
static void cmd_fault_test(int argc, char **argv)
{
    int idx = 1;   /* default: task 1 (Counter) */
    if (argc > 1) {
        idx = (int)strtoul(argv[1], NULL, 10);
    }
    if (idx < 0 || idx >= MAX_TASKS || tasks[idx].name == NULL) {
        printf("Usage: fault_test [task_index]  (0-%d)\r\n", MAX_TASKS - 1);
        return;
    }
    printf("Corrupting stack[0] of task %d (%s) ...\r\n",
           idx, tasks[idx].name);
    /* XOR flips bits but keeps the value non-zero and non-canary, so the
     * guard check in pendsv_switch() will reliably fail.               */
    tasks[idx].stack[0] ^= 0xBAD00BADu;
    printf("Done. Fault detected on next context switch from that task.\r\n");
    printf("Check the display - a red banner will appear.\r\n");
}
