...
static volatile bool btn_raw[BUTTON_COUNT] = {0};
static volatile bool btn_debounced[BUTTON_COUNT] = {0};
static volatile bool btn_prev_debounced[BUTTON_COUNT] = {0};
static uint32_t btn_last_change[BUTTON_COUNT] = {0};
...
disp_error_t disp_init(const disp_config_t *cfg) {
    ...
    printf("DEBUG: Initializing SPI at %lu Hz\n", (unsigned long)g.config.spi_baudrate);
    ...
    g.initialized = true;
    printf("DEBUG: Display initialized successfully\n");
    return DISP_OK;
}
...
// Buttons - FIXED LOGIC with proper debouncing
disp_error_t buttons_init(void) {
    ...
    for (int i = 0; i < BUTTON_COUNT; i++) {
        ...
        btn_raw[i] = pressed;
        btn_debounced[i] = pressed;
        btn_prev_debounced[i] = pressed;
        btn_last_change[i] = 0;
    }
    buttons_ready = true;
...
void buttons_update(void) {
    ...
    for (int i = 0; i < BUTTON_COUNT; i++) {
        // Save previous debounced state BEFORE any changes
        btn_prev_debounced[i] = btn_debounced[i];
        ...
        // Debouncing logic
        if (pressed != btn_debounced[i]) {
            // Raw state differs from debounced state
            if (btn_last_change[i] == 0) {
                // Start debounce timer
                btn_last_change[i] = now;
            } else if (now - btn_last_change[i] >= DEBOUNCE_MS) {
                // Stable for debounce period - accept the change
                bool was_pressed = btn_debounced[i];
                btn_debounced[i] = pressed;
                btn_last_change[i] = 0;
                
                // Trigger callback on press (rising edge)
            ...
            // Raw matches debounced - reset timer
            btn_last_change[i] = 0;
        }
    }
}
