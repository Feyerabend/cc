...
static const char* error_strings[] = {
    "OK", "Init failed", "DMA operation failed", 
    "Invalid parameter", "Display not initialised"
};
...
static bool dma_wait_for_finish_timeout(uint32_t timeout_ms) {
    ...
    while (dma_busy) {
        if (get_time_ms() - start > timeout_ms) return false;
        tight_loop_contents();
    }
    return true;
}
...
static display_error_t dma_spi_write_buffer(uint8_t* data, size_t len) {
    ...
    dma_channel_configure(dma_channel, &c, &spi_get_hw(spi0)->dr, data, len, false);
...
static display_error_t display_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    ...
    if ((result = display_write_command(0x2A)) != DISPLAY_OK) return result;
    ...
    if ((result = display_write_command(0x2B)) != DISPLAY_OK) return result;
    ...
    if ((result = display_write_command(0x2C)) != DISPLAY_OK) return result;
...
display_error_t display_pack_init(void) {
    ...
    display_error_t result;
    if ((result = display_write_command(0x01)) != DISPLAY_OK) goto init_error;
    sleep_ms(150);
    
    if ((result = display_write_command(0x11)) != DISPLAY_OK) goto init_error;
    sleep_ms(120);
    
    if ((result = display_write_command(0x3A)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x55)) != DISPLAY_OK) goto init_error;
    
    if ((result = display_write_command(0x36)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x70)) != DISPLAY_OK) goto init_error;
    
    if ((result = display_write_command(0x2A)) != DISPLAY_OK) goto init_error;
    ...
    if ((result = display_write_command(0x2B)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x00)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x00)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x00)) != DISPLAY_OK) goto init_error;
    ...
    display_write_data(0x0C); display_write_data(0x0C); display_write_data(0x00);
    display_write_data(0x33); display_write_data(0x33);
    
    display_write_command(0xB7); display_write_data(0x35);
    display_write_command(0xBB); display_write_data(0x19);
    display_write_command(0xC0); display_write_data(0x2C);
    display_write_command(0xC2); display_write_data(0x01);
    display_write_command(0xC3); display_write_data(0x12);
    display_write_command(0xC4); display_write_data(0x20);
    display_write_command(0xC6); display_write_data(0x0F);
    display_write_command(0xD0); display_write_data(0xA4); display_write_data(0xA1);
...
display_error_t display_blit_full(const uint16_t *pixels) {
    ...
    // Send in chunks to work around any size limitations
    const size_t chunk_size = 32768; // 16K pixels at a time
    const size_t total_bytes = DISPLAY_WIDTH * DISPLAY_HEIGHT * 2;
    size_t offset = 0;
    
    while (offset < total_bytes) {
        size_t bytes_to_send = (total_bytes - offset > chunk_size) ? chunk_size : (total_bytes - offset);
        result = dma_spi_write_buffer((uint8_t *)pixels + offset, bytes_to_send);
        if (result != DISPLAY_OK) break;
        dma_wait_for_finish();
