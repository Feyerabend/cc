...
# Primitive types that are always valid return/variable types
_PRIMITIVE_TYPES = {"i32", "i64", "u32", "u64", "bool", "unit", "List"}
...
class StructDef:
    ...
    tag: int = 0  # runtime tag for pattern-match dispatch

    def to_c(self) -> str:
        c_name = sanitize_name(self.name)
        # Always emit _tag first so all structs share a common initial sequence,
        # which makes void*-based tag reads defined behaviour (C11 §6.5.2.3/6).
        lines = ["int _tag;"] + [f"{t.to_c()} {sanitize_name(f)};" for f, t in self.fields.items()]
        body = " ".join(lines)
...
class Var:
    def __init__(self, name: str, typ: Type, ownership: Ownership):
        ...
        self.borrowed_from: Optional['Var'] = None  # Source variable this borrows from
...
class Compiler:
    def __init__(self):
        ...
        self.lt_counter = 0
        self.struct_tag_counter = 0  # Assigns unique tags to structs for match dispatch
    ...
    def parse_type(self, expr: Any) -> Type:
        ...
        self.errors.append(f"ERROR: Invalid type expression: {expr!r}")
        return Type("i32")  # fallback to avoid cascading errors
    ...
    def generate_derived_trait_impl(self, struct_name: str, trait_name: str):
        ...
        if trait_name == "Copy":
            ...
            self.codegen.emit(f"result._tag = self->_tag;")
    ...
    def compile_expr(self, expr: Any, value_needed: bool = True) -> Tuple[str, Type]:
        ...
        if op in self.structs:
            ...
            self.codegen.emit(f"{temp}._tag = {struct.tag};")  # set runtime tag
        ...
        if op == "let":
            ...
            for bind in bindings:
                ...
                new_var = Var(c_name, val_typ, ownership)
                # Track which source variable this borrow came from so we can
                # release the borrow count when this scope exits.
                if ownership in (Ownership.BORROW, Ownership.BORROW_MUT):
                    if isinstance(val_expr, list) and val_expr and val_expr[0] == "borrow":
                        borrow_target = val_expr[1]
                        if isinstance(borrow_target, str):
                            new_var.borrowed_from = old_scope.lookup(sanitize_name(borrow_target))
            ...
            # Release borrow counts when scope exits
            for var in self.scope.vars.values():
                if var.borrowed_from is not None:
                    if var.ownership == Ownership.BORROW:
                        var.borrowed_from.borrowed_immut_count = max(0, var.borrowed_from.borrowed_immut_count - 1)
                    elif var.ownership == Ownership.BORROW_MUT:
                        var.borrowed_from.borrowed_mut = False
        ...
        if op == "if":
            if len(expr) < 4:
                self.errors.append("ERROR: 'if' requires condition, then-branch, and else-branch")
                return "0", Type("i32")
        ...
        ops = {"==": "==", "!=": "!=", "<": "<", ">": ">", "<=": "<=", ">=": ">=",
               "+": "+", "-": "-", "*": "*", "/": "/", "%": "%"}
        if op in ops:
            if op in ("/", "%") and isinstance(expr[2], int) and expr[2] == 0:
                self.errors.append("ERROR: Division by zero")
        ...
        if op == "match":
            ...
            # Determine a pointer expression suitable for reading the _tag field
            # and casting to the concrete struct type during field access.
            #
            #   (& SomeStruct) → C type is SomeStruct*; dereference to get the struct ptr
            #   (& List)       → C type is void* (List=void*); already the node ptr, no deref
            #   SomeStruct val → C value type; take address for pointer-cast operations
            #   List / void*   → already the node pointer; use directly
            if val_typ.is_ref and val_typ.name != "List":
                ptr_expr = f"(*{val_code})"
            elif val_typ.name in self.structs:
                ptr_expr = f"(&{val_code})"
            else:
                ptr_expr = val_code

            def make_tag_check(tag: int) -> str:
                return f"((struct{{int _tag;}}*){ptr_expr})->_tag == {tag}"

            def make_field_access(pat_kind: str, f_cname: str) -> str:
                # Direct struct value: plain member access avoids an extra indirection.
                if not val_typ.is_ref and val_typ.name in self.structs:
                    return f"{val_code}.{f_cname}"
                return f"(({sanitize_name(pat_kind)}*){ptr_expr})->{f_cname}"

            if not value_needed:
                first_if = True
                for case in expr[2:]:
                    if not isinstance(case, list) or len(case) < 2:
                        self.errors.append("ERROR: Invalid match case")
                        continue
                    pat, body = case[0], case[1]
                    if not isinstance(pat, list) or not pat:
                        self.errors.append("ERROR: Invalid match pattern")
                        continue
                    pat_kind = pat[0]
                    struct = self.structs.get(pat_kind)
                    if struct is None:
                        self.errors.append(f"ERROR: Unknown pattern variant '{pat_kind}'")
                        continue
                    old_scope = self.scope
                    self.scope = Scope(old_scope)
                    if first_if:
                        self.codegen.emit(f"if ({make_tag_check(struct.tag)}) {{")
                        first_if = False
                    else:
                        self.codegen.emit(f"}} else if ({make_tag_check(struct.tag)}) {{")
                    self.codegen.indent += 1
                    field_names = list(struct.fields.keys())
                    for i, pat_var in enumerate(pat[1:]):
                        if pat_var == "_" or i >= len(field_names):
                            continue
                        f_name = field_names[i]
                        field_typ = struct.fields[f_name]
                        f_cname = sanitize_name(f_name)
                        access = make_field_access(pat_kind, f_cname)
                        v_cname = sanitize_name(pat_var)
                        self.codegen.emit(f"{field_typ.to_c()} {v_cname} = {access};")
                        self.scope.add(Var(v_cname, field_typ, Ownership.BORROW))
            ...
            result_temp = self.codegen.new_temp()
            self.codegen.emit(f"int {result_temp} = 0;")
            first_if = True
            for case in expr[2:]:
                if not isinstance(case, list) or len(case) < 2:
                    self.errors.append("ERROR: Invalid match case")
                ...
                self.scope = old_scope
            if first_if:
                self.errors.append("ERROR: Empty match expression")
        ...
        if op in self.functions:
            ...
            if len(expr) - 1 != len(param_types):
                self.errors.append(
                    f"ERROR: Function '{op}' expects {len(param_types)} argument(s), got {len(expr) - 1}"
                )
        args_codes = []
    ...
    def compile_defstruct(self, form):
        ...
        self.struct_tag_counter += 1
        self.structs[name] = StructDef(name, fields, derives, tag=self.struct_tag_counter)
    ...
    def compile_defn(self, form, prefix: str = ""):
        ...
        # Distinguish a return-type annotation from the first body expression.
        # We only treat form[3] as a type if it names a known primitive or struct,
        # or is a reference type list starting with "&". This prevents a plain
        # variable reference (e.g. `x`) from being mistaken for a return type.
        if len(form) > 3:
            next_elem = form[3]
            known_types = _PRIMITIVE_TYPES | set(self.structs.keys())
            if isinstance(next_elem, str) and next_elem in known_types:
