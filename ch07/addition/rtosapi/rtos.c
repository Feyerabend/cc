...
void rtos_stats_get(rtos_stats_t *out)
{
    ...
    for (uint8_t i = 0; i < num_tasks; i++) {
         ...
         * the stack base (lowest address) upward.                   */
        uint16_t free_words = 0;
        while (free_words < TASK_STACK_SIZE &&
               tcb->stack[free_words] == RTOS_STACK_CANARY) {
            free_words++;
        }
        s->stack_peak_words = TASK_STACK_SIZE - free_words;
    }
}
