...
class RISCVAssembler:
    ...
    def parse_immediate(self, imm: str) -> int:
        """Parse immediate value (decimal or hex, can be negative, or .equ constant)"""
    ...
    def _parse_string_literal(self, raw: str) -> bytes:
        """Parse a quoted string literal, handling escape sequences."""
        ...
        while i < len(raw):
            if raw[i] == '\\' and i + 1 < len(raw):
                esc = raw[i+1]
                if esc == 'n':  result.append(10)
                elif esc == 't': result.append(9)
                elif esc == 'r': result.append(13)
                elif esc == '0': result.append(0)
                elif esc == '\\': result.append(92)
                elif esc == '"':  result.append(34)
                else:             result.append(ord(esc))
                i += 2
            else:
    ...
    def assemble(self, source: str) -> bytes:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Handle .equ before label detection (it has no address)
            if line.startswith('.equ'):
                ...
                if len(parts) >= 3:
                    ...
                    try:
                        ...
                        val = self.labels.get(val_str, 0)
                    self.labels[name] = val
                    if self.verbose:
                        print(f"Constant '{name}' = {val}")
                continue

            # Check for label (only if line doesn't start with '.' directive)
                ...
                if rest:
                    ...
                    if op == 'LA':
                        ...
                        if directive == '.word':
                            ...
                            raw = rest[len(directive):].strip()
                            s = self._parse_string_literal(raw)
                            addr += len(s) + (1 if directive == '.asciz' else 0)
                    else:
                        addr += 4
            else:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Strip label if present (only for non-directive lines)
            if ':' in line and not line.startswith('.'):
                parts = line.split(':', 1)
            ...
            # Skip .equ lines (handled in first pass)
            if line.startswith('.equ'):
                continue
            ...
            if line.startswith('.'):
                ...
                if directive == '.word':
                    ...
                    self.source_map.append((line_num, original_line))
                    if self.verbose:
                        print(f"0x{addr:04x}: {directive} {raw!r:30s} => {len(s)} bytes")
                    addr += len(s)
