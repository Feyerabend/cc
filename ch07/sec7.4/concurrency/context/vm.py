...
class RISCVVM:
    ...
    def handle_syscall(self):
        ...
        elif syscall_num == 64:  # YIELD / Context switch visualisation
            self._show_context_switch()
        else:
            if self.debug:
                print(f"Unknown syscall: {syscall_num}")

    def _show_context_switch(self):
        """
        Visualise a cooperative context switch.
        
        At call time:
          a0 (x10) = address of CURRENT task's context block (being saved / just saved)
          a1 (x11) = address of NEXT task's context block (being restored)
        
        Context block layout (10 words):
          [0]  s0   [4]  s1   [8]  s2   [12] s3
          [16] s4   [20] s5   [24] s6   [28] s7
          [32] sp   [36] ra  (resume PC)
        """
        ctx_cur = self.regs[10]   # a0
        ctx_nxt = self.regs[11]   # a1

        def read_ctx(base):
            if base == 0 or base + 40 > len(self.memory):
                return None
            import struct
            words = struct.unpack_from('<10I', self.memory, base)
            names = ['s0', 's1', 's2', 's3', 's4', 's5', 's6', 's7', 'sp', 'ra']
            return dict(zip(names, words))

        cur = read_ctx(ctx_cur)
        nxt = read_ctx(ctx_nxt)
        ...
        if cur:
            print(f"│  {'SAVE  (current task @ 0x' + f'{ctx_cur:04x}' + ')'}")
            print(f"│  {'s0':>3}=0x{cur['s0']:08x}  {'s1':>3}=0x{cur['s1']:08x}  {'s2':>3}=0x{cur['s2']:08x}")
            print(f"│  {'s3':>3}=0x{cur['s3']:08x}  {'s4':>3}=0x{cur['s4']:08x}  {'s5':>3}=0x{cur['s5']:08x}")
            print(f"│  {'s6':>3}=0x{cur['s6']:08x}  {'s7':>3}=0x{cur['s7']:08x}")
            print(f"│  {'sp':>3}=0x{cur['sp']:08x}  {'ra':>3}=0x{cur['ra']:08x}  (resume PC)")
        else:
            print(f"│  {'SAVE  (scheduler / no context)'}")

        print()
        print(f"│  {'  v  switching  v  '}")
        print(f"{'   -------------------'}")


        if nxt:
            print(f"│  {'RESTORE (next task @ 0x' + f'{ctx_nxt:04x}' + ')'}")
            print(f"│  {'s0':>3}=0x{nxt['s0']:08x}  {'s1':>3}=0x{nxt['s1']:08x}  {'s2':>3}=0x{nxt['s2']:08x}")
            print(f"│  {'s3':>3}=0x{nxt['s3']:08x}  {'s4':>3}=0x{nxt['s4']:08x}  {'s5':>3}=0x{nxt['s5']:08x}")
            print(f"│  {'s6':>3}=0x{nxt['s6']:08x}  {'s7':>3}=0x{nxt['s7']:08x}")
            print(f"│  {'sp':>3}=0x{nxt['sp']:08x}  {'ra':>3}=0x{nxt['ra']:08x}  (resume PC)")
        else:
            print(f"│  {'RESTORE (no context)':}")
