...
class Continuation(BaseException):
    """
    First-class continuation for call/cc (call-with-current-continuation).
    Captures the current execution context and allows non-local exits.
    Inherits from BaseException to be used with Python's exception mechanism.
    """
    ...
    def __call__(self, arg: Any) -> Any:
        """Invoking a continuation escapes to the captured point"""
        if self.escaped:
            raise ErrorHandler.runtime_error("Continuation already used")
        self.escaped = True
        self.value = arg
        raise self  # Use exception mechanism for non-local exit
...
class Procedure:
    ...
    def __call__(self, *args) -> Any:
        ...
        while True:
            ...
            except Continuation as cont:
                # A continuation was invoked - propagate it up
                raise
...
class CallCCEvaluator(Evaluator):
    """
    call-with-current-continuation (call/cc) evaluator.
    Captures the current continuation and passes it to a procedure.
    """
    def evaluate(self, expr: List[Any], env: Environment, interpreter: 'Lisp', tail_context: bool = False) -> Any:
        if len(expr) != 2:
            raise ErrorHandler.runtime_error("call/cc requires exactly 1 argument (a procedure)")
        
        # Evaluate the procedure argument
        proc = interpreter.eval(expr[1], env)
        
        if not callable(proc):
            raise ErrorHandler.runtime_error("call/cc argument must be a procedure")
        
        # Create a continuation that captures the current execution point
        ...
        try:
            ...
            result = proc(cont)
            return result
        except Continuation as escaped_cont:
            # The continuation was invoked - return its value
            if escaped_cont is cont:
                return escaped_cont.value
            else:
                # Re-raise if it's a different continuation
                raise
...
class Lisp:
    ...
    def _setup_special_forms(self):
        ...
        self.special_forms = {
            ...
            'call/cc': CallCCEvaluator(),
            'call-with-current-continuation': CallCCEvaluator(),
        }
...
I/O:           print, display
Special forms: quote, if, cond, and, or, define, set!, lambda, let, begin, while, call/cc
