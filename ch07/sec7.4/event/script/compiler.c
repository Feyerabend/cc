...
static void compile_stmt(ASTNode *node, Bytecode *bc) {
    switch (node->type) {
        ...
        case AST_TIMER_AFTER: {
            // Push delay (milliseconds)
            Value delay = {.type = VAL_NUMBER, .number = node->timer.delay_ms};
            size_t delay_idx = bytecode_add_constant(bc, delay);
            bytecode_write(bc, OP_CONST);
            bytecode_write(bc, (uint8_t)delay_idx);
            
            // Push event name
            Value event_name = {.type = VAL_STRING, .string = strdup(node->timer.event_name)};
            size_t name_idx = bytecode_add_constant(bc, event_name);
            bytecode_write(bc, OP_CONST);
            bytecode_write(bc, (uint8_t)name_idx);
            
            // Push data
            compile_expr(node->timer.data, bc);
            
            // Timer instruction
            bytecode_write(bc, OP_TIMER);
            break;
        }
