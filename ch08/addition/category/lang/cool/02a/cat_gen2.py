    ...
    def type_check(self, env, type_env):
        ...
        if self.op == '+':
            if left_type == right_type == INT_TYPE:
                ...
                raise TypeError(f"Operator + requires matching Int or String operands")
        elif self.op in ['-', '*', '/']:
...
def demo():
    ...
    # Example 8: Simple Monoids
    print("\n  Example 8: Simple Monoids (StringMonoid and Sum)")
    print("-" * 70)
    
    # Define StringMonoid
    string_monoid = ClassDef("StringMonoid")
    string_monoid.add_field(Field("value", STRING_TYPE, ""))
    
    str_combine_body = BlockStatement([
        VarDecl("result", "StringMonoid", [], NewObject("StringMonoid")),
        Assignment(Variable("result"), "value", BinaryOp("+", FieldAccess(Variable("this"), "value"), FieldAccess(Variable("other"), "value"))),
        ReturnStatement(Variable("result"))
    ])
    
    string_monoid.add_method(Method("combine", ["other"], [string_monoid], string_monoid, str_combine_body))
    
    type_env.register_type(string_monoid)
    
    print(f"    Defined class: {string_monoid.name()}")
    print(f"    Field: value: String = \"\"")
    print(f"    Method: combine(other: StringMonoid): StringMonoid")
    
    # StringMonoid demonstration program
    str_demo_stmt = BlockStatement([
        VarDecl("empty_str", "StringMonoid", [], NewObject("StringMonoid")),
        VarDecl("hello", "StringMonoid", [], NewObject("StringMonoid")),
        Assignment(Variable("hello"), "value", StringLiteral("Hello")),
        VarDecl("world", "StringMonoid", [], NewObject("StringMonoid")),
        Assignment(Variable("world"), "value", StringLiteral(", world!")),
        VarDecl("combined", "StringMonoid", [], MethodCall(Variable("empty_str"), "combine", [Variable("hello")])),
        VarDecl("combined2", "StringMonoid", [], MethodCall(Variable("combined"), "combine", [Variable("world")])),
        PrintStatement(FieldAccess(Variable("combined2"), "value"))
    ])
    
    print("\n    StringMonoid demonstration output:")
    str_demo_stmt.execute(env)
    
    # Define Sum
    sum_class = ClassDef("Sum")
    sum_class.add_field(Field("value", INT_TYPE, 0))
    
    sum_combine_body = BlockStatement([
        VarDecl("result", "Sum", [], NewObject("Sum")),
        Assignment(Variable("result"), "value", BinaryOp("+", FieldAccess(Variable("this"), "value"), FieldAccess(Variable("other"), "value"))),
        ReturnStatement(Variable("result"))
    ])
    
    sum_class.add_method(Method("combine", ["other"], [sum_class], sum_class, sum_combine_body))
    
    type_env.register_type(sum_class)
    
    print(f"\n    Defined class: {sum_class.name()}")
    print(f"    Field: value: Int = 0")
    print(f"    Method: combine(other: Sum): Sum")
    
    # Sum demonstration program
    sum_demo_stmt = BlockStatement([
        VarDecl("zero", "Sum", [], NewObject("Sum")),
        VarDecl("seven", "Sum", [], NewObject("Sum")),
        Assignment(Variable("seven"), "value", IntLiteral(7)),
        VarDecl("forty_two", "Sum", [], NewObject("Sum")),
        Assignment(Variable("forty_two"), "value", IntLiteral(42)),
        VarDecl("total", "Sum", [], MethodCall(Variable("zero"), "combine", [Variable("seven")])),
        VarDecl("total2", "Sum", [], MethodCall(Variable("total"), "combine", [Variable("forty_two")])),
        PrintStatement(FieldAccess(Variable("total2"), "value"))
    ])
    
    print("\n    Sum demonstration output:")
    sum_demo_stmt.execute(env)
    ...
    print("  Simple Monoids: String concatenation and integer addition")
    print("-" * 70)
