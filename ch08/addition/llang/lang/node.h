...
/* Maximum cases in ND_INDREC: ch[0]=scrut, ch[1]=motive, ch[2..]=cases. */
#define ND_MAX_CASES     8
/* Total ch[] slots per node: 2 fixed (scrut/motive) + ND_MAX_CASES.
 * ND_INDCON and ND_INDTYPE also use ch[0..n-1] and are bounded by this. */
#define NODE_MAX_CHILDREN (ND_MAX_CASES + 2)
...
typedef enum {
    ...
    ND_QUOTREC,      /* ch[0]=ty_a, ch[1]=rel, ch[2]=ty_b, ch[3]=func, ch[4]=coh, ch[5]=scrut */
...
typedef struct {
    ...
    uint8_t  flags;
    uint16_t _pad;
    NodeRef  ch[NODE_MAX_CHILDREN];  /* ch[0]=scrut/fun, ch[1]=motive/arg, ch[2..]=cases */
...
NodeRef mk_quot  (Heap *h);
NodeRef mk_qin   (Heap *h);
