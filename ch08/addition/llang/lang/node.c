...
NodeRef mk_quot (Heap *h) { return mk0(h, ND_QUOT,  NF_WHNF | NF_NF); }
NodeRef mk_qin  (Heap *h) { return mk0(h, ND_QIN,   NF_WHNF | NF_NF); }
...
static const char *tag_name(NodeTag t) {
    ...
    case ND_QUOT:         return "QUOT";
    case ND_QIN:          return "QIN";
    ...
    case ND_QUOTREC:      return "QUOTREC";
...
int node_is_elim_tag(NodeTag t) {
    ...
    case ND_TRUNCREC: case ND_QUOTREC: case ND_CASESPLIT: case ND_ABORT:
...
NodeRef node_scrut_of(Heap *h, NodeRef r) {
    ...
    case ND_QUOTREC:  return h->nodes[r].ch[5];
...
static void print_elim_frame(Heap *h, NodeRef r, int depth) {
    ...
    case ND_INDREC: {
        ...
        if (fam->n_ctors > ND_MAX_CASES) { printf("<indrec:%s/overflow>", fam->name); break; }
...
void node_print(Heap *h, NodeRef r, int depth, int prec) {
    ...
    case ND_QUOT:  printf("Quot");  break;
    case ND_QIN:   printf("qin");   break;
    case ND_LAM:
    ...
    case ND_GLOBAL:
        ...
        case -4:  printf("qeq");    break;
        case INTERVAL_CONST_LVL: printf("II"); break;
        case IZERO_CONST_LVL:    printf("i0"); break;
        case IONE_CONST_LVL:     printf("i1"); break;
    ...
    case ND_UNI_V:
        ...
        break;
    case ND_CORE: {
        /* Quote the embedded Val* back to a term and print it. */
        Arena tmp = {NULL};
        Val  *v   = (Val *)h->nodes[r].aux;
        Term *t   = nbe_quote(&tmp, 0, v);
        if (t) term_fprint_ctx(stdout, t, NULL, prec);
        else   printf("<core>");
        arena_free_all(&tmp);
        break;
    }
...
static int term_eq(const Term *t1, const Term *t2) {
    ...
    case TM_QUOTREC:
        return term_eq(t1->quotrec_t.ty_a,  t2->quotrec_t.ty_a)  &&
               term_eq(t1->quotrec_t.rel,   t2->quotrec_t.rel)   &&
               term_eq(t1->quotrec_t.ty_b,  t2->quotrec_t.ty_b)  &&
               term_eq(t1->quotrec_t.func,  t2->quotrec_t.func)  &&
               term_eq(t1->quotrec_t.coh,   t2->quotrec_t.coh)   &&
               term_eq(t1->quotrec_t.scrut, t2->quotrec_t.scrut);
    case TM_CIRCREC:
    ...
    case TM_LZERO:
        return 1;
    case TM_LSUC:
    case TM_ISONE:  /* Phase L2 Stage 7 */
        return term_eq(t1->elim, t2->elim);
    case TM_PRIMSUB:
        return term_eq(t1->primsub_t.ty,   t2->primsub_t.ty)   &&
               term_eq(t1->primsub_t.face, t2->primsub_t.face) &&
               term_eq(t1->primsub_t.u,    t2->primsub_t.u)    &&
               term_eq(t1->primsub_t.a,    t2->primsub_t.a);
    case TM_LMAX:
        return term_eq(t1->app.fun, t2->app.fun) && term_eq(t1->app.arg, t2->app.arg);
    case TM_UNI_V:
        return term_eq(t1->uni_v_lvl, t2->uni_v_lvl);
    case TM_MATCH: {  /* Phase M4 */
        if (t1->match_s.fam_idx != t2->match_s.fam_idx) return 0;
        if (t1->match_s.n_arms  != t2->match_s.n_arms)  return 0;
        if (!term_eq(t1->match_s.scrut, t2->match_s.scrut)) return 0;
        for (int i = 0; i < t1->match_s.n_arms; i++)
            if (!term_eq(t1->match_s.arms[i].body, t2->match_s.arms[i].body)) return 0;
        return 1;
    }
    default: return 0;
    }
...
int node_conv(Heap *h, Arena *a, NodeRef r1, NodeRef r2) {
    ...
    case ND_LAM:
        return h->nodes[r1].aux == h->nodes[r2].aux &&
               node_conv_env(h, a, h->nodes[r1].ch[0], h->nodes[r2].ch[0]);
    /* Core Val*: try pointer equality first, fall back to core NbE conv.
     * Pointer equality is not sufficient for separately allocated VL_ISONE,
     * VL_TRANSP, VL_HCOMP, etc. that represent the same stuck neutral. */
    case ND_CORE:
        if (h->nodes[r1].aux == h->nodes[r2].aux) return 1;
        return conv(a, 0, (Val *)h->nodes[r1].aux, (Val *)h->nodes[r2].aux);
    ...
    case ND_INDREC: {
        ...
        int fam_idx = h->nodes[r1].lvl;
        IndDef *fam = ind_get(fam_idx);
        if (fam->n_ctors > ND_MAX_CASES) return 0;
