...
static Term *node_to_term_ctx(Heap *h, NodeRef r, Arena *a, SentCtx *ctx) {
    ...
    case ND_QUOT:  return tm_quot(a);
    case ND_QIN:   return tm_qin(a);
    ...
    case ND_GLOBAL: {
        ...
        if (idx == -4) return tm_qeq(a);
        if (idx == INTERVAL_CONST_LVL) return tm_interval(a);
        if (idx == IZERO_CONST_LVL)    return tm_izero(a);
        if (idx == IONE_CONST_LVL)     return tm_ione(a);
        return NULL;
    }
    ...
    case ND_QUOTREC: {
        Term *ty_a  = node_to_term_ctx(h, h->nodes[r].ch[0], a, ctx);
        Term *rel   = node_to_term_ctx(h, h->nodes[r].ch[1], a, ctx);
        Term *ty_b  = node_to_term_ctx(h, h->nodes[r].ch[2], a, ctx);
        Term *func  = node_to_term_ctx(h, h->nodes[r].ch[3], a, ctx);
        Term *coh   = node_to_term_ctx(h, h->nodes[r].ch[4], a, ctx);
        Term *scrut = node_to_term_ctx(h, h->nodes[r].ch[5], a, ctx);
        return (ty_a && rel && ty_b && func && coh && scrut)
               ? tm_quotrec(a, ty_a, rel, ty_b, func, coh, scrut) : NULL;
    }
    case ND_CASESPLIT: {
        NodeRef c0 = h->nodes[r].ch[0], c1 = h->nodes[r].ch[1];
        NodeRef c2 = h->nodes[r].ch[2], c3 = h->nodes[r].ch[3];
    ...
    case ND_INDTYPE: {
        ...
        if (n > 6) {
            fprintf(stderr, "bridge: indtype '%s' has %d args (cap is 6); cannot serialize\n",
                    fam->name, n);
            return NULL;
        }
    ...
    case ND_INDCON: {
        ...
        if (n > 6) {
            fprintf(stderr, "bridge: indcon '%s' has %d args (cap is 6); cannot serialize\n",
                    fam->ctors[ctor_idx].name, n);
            return NULL;
        }
        Term **args  = n > 0 ? (Term **)arena_alloc(a, n * sizeof(Term *)) : NULL;
        for (int i = 0; i < n; i++) {
    ...
    case ND_INDREC: {
        ...
        int n       = fam->n_ctors;
        if (n > 4) {
            fprintf(stderr, "bridge: indrec for '%s' has %d ctors (cap is 4); cannot serialize\n",
                    fam->name, n);
            return NULL;
        }
