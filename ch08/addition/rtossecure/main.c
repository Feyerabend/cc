...
#define COL_RED      0x00F8u   /* fault overlay (byte-swapped 0xF800) */
...
static void core1_display_main(void) {
    ...
    while (1) {
        ...
        /* -- Fault overlay (drawn last so it is always on top) -- */
        if (rtos_last_fault.active) {
            /* Red banner over the timeline area */
            fb_fill_rect(framebuf, 0, TL_Y, DISPLAY_WIDTH, 40, COL_RED);
            fb_draw_string(framebuf, 8, TL_Y + 4,
            ...
            fb_draw_string(framebuf, 8, TL_Y + 18,
                           fault_buf, COLOR_WHITE, COL_RED);
        }
